...
void ForceDirectedAdjustment(const Graph& g, map<string, SegmentTree*>& trees);
...
public:
	RoutingGraph(const Graph& g, const map<string, SegmentTree*>& trees)
	{
	...
	bool HubAvoidsObstacles(RoutingNode* node, const Point& nodePosition, double idealR, vector<Point>& touchedObstacles)
	{
		...
		for (int i = 0; i < (int)hardObstacles.size(); i++)
		{
			...
			Point closestP = ClosestPoint(hardObstacles[i], nodePosition);
		...
		for (int i = 0; i < (int)nodes.size(); i++)
		{
			...
			for (int j = 0; j < (int)nodes[i]->neighbors.size(); j++)
			{
				...
				Point closestP = ClosestPoint(seg2, nodePosition);
		...
		double idealDist, vector<pair<Point, Point> >& touchedObstacles)
	{
		...
		for (int i = 0; i < (int)hardObstacles.size(); i++)
		{
			...
			Point p1, p2;
			if (Intersect(hardObstacles[i], Segment(nodePosition, adjPosition), p1, p2)) return false;
