...
Greedy::Greedy(int nnode, double deg, IMNode** ah)
{
	Nnode = nnode;
	node = ah;
	degree = deg;
...
void Greedy::move(bool &moved)
{
	...
	{
		int randPos = i + randInt(Nnode - i - 1);
	...
	for (int k = 0; k < Nnode; k++)
	{
		...
		{
			int randPos = j + randInt(NmodLinks - j - 1);
...
void Greedy::initiate(void)
{
	...
	for (int i = 0; i < Nnode; i++)
	{
		...
		Mdeg += node[i]->outDegree; // If node connects to nodes outside branch
		node[i]->exit = Mdeg;
		node[i]->degree = Mdeg; //Update when self-links exist
		outDegree += node[i]->outDegree; // If node connects to nodes outside branch
	}
...
void Greedy::tune(void)
{
	...
	for (int i = 0; i < Nnode; i++)
	{
		...
		mod_members[i_M]++;
		mod_degree[i_M] += i_d;
		mod_outDegree[i_M] += node[i]->outDegree; // If module connects to nodes outside branch
		...
		mod_exit[i_M] += node[i]->outDegree; // If module connects to nodes outside branch
	}
	...
	exitDegree += outDegree;
	exit = plogp(exitDegree);
	indexLength = exit - out_log_out - exit_log_exit;
	moduleLength = -exit_log_exit + degree_log_degree - nodeDegree_log_nodeDegree;
	codeLength = exit - out_log_out - 2.0 * exit_log_exit + degree_log_degree - nodeDegree_log_nodeDegree;
}
...
void Greedy::calibrate(void)
{
	...
	vector<double>(Nmod).swap(mod_outDegree);
	...
	for (int i = 0; i < Nmod; i++)
	{
		...
		mod_outDegree[i] = node[i]->outDegree;
		mod_members[i] = node[i]->members.size();
		node[i]->index = i;
		mod_outDegree[i] = node[i]->outDegree;  // If module connects to nodes outside branch
	}
...
void Greedy::level(IMNode ***node_tmp, bool sort)
{
	prepare(sort);
	(*node_tmp) = new IMNode*[Nmod];
	vector<int> nodeInMod(Nnode);

	for (int i = 0; i < Nmod; i++)
	{
		(*node_tmp)[i] = new IMNode();
		(*node_tmp)[i]->index = i;
		(*node_tmp)[i]->exit = mod_exit[modWnode[i]];
		(*node_tmp)[i]->degree = mod_degree[modWnode[i]];
		(*node_tmp)[i]->outDegree = mod_outDegree[modWnode[i]];
