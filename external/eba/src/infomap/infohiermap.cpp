...
struct InfomapResult
{
	...
	map<Node*, string> code;

	vector<vector<Node*> > extractResult(ConnectedGraph& g)
	{
		map<string, vector<Node*> > cl;
	...
	vector<vector<Node*> > extractDisconnectedResult(ConnectedGraph& g)
	{
		vector<vector<Node*> > res;
		for (int i = 0; i < (int)g.nodes.size(); i++)
		{
